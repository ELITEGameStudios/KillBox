...
[System.Serializable]
public class MidasShootState : BossStateData
{
    MidasBoss midasData;
    ...
    public MidasShootState(MidasBoss bossBase, float time, float speed, float force, float dist, float fr, float init) : base(bossBase){
        midasData = bossBase;
        this.stateTime = time;
        this.maxSpeed = speed; 
        this.accel = force;
    ...
    void FiringUpdate(){
        ...
        {
            foreach (AIShooterScript shooter in midasData.crownShooter){
                shooter.Shoot();
            }
