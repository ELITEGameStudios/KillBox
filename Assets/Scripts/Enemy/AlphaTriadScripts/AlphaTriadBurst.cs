...
[System.Serializable]
public class AlphaTriadBurst : BossStateData
{
    ...
    public AlphaTriadBurst(AlphaTriadBoss bossBase, float stateTime, float speed, float force, float fireRate) : base(bossBase){
        triadData = bossBase;
    ...
    void FiringUpdate(){
        if (currentFireInterval <= 0){
            foreach(AIShooterScript source in triadData.shoot_sources){
                source.Shoot();
            }
            currentFireInterval = fireRate;
        }
