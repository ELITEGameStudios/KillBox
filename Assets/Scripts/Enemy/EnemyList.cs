...
public class EnemyList : MonoBehaviour
{
    ...
    //         if(manager.LvlCount > 12 && manager.LvlCount < 21){

    //             if(manager.CurrentBossDif < boss_dif[1]){ return null; }
    //             manager.CurrentBossDif -= boss_dif[1];
    //             return mini_boss_types[1];
    //         }
    //         else if(manager.LvlCount == 24){
    //             if(manager.CurrentBossDif < boss_dif[2]){ return null; }
    //             manager.CurrentBossDif -= boss_dif[2]; 
    //             return mini_boss_types[2];
    //         }

    //         List<GameObject> valid_boses = new List<GameObject>();
    //         List<float> valid_dif = new List<float>();

    //         for(int i = 0; i < mini_boss_types.Length; i++){

    //             if(mini_boss_allowed.Length > i){

    //                 if(manager.LvlCount >= mini_boss_allowed[i] && manager.CurrentBossDif - boss_dif[i] >= 0){
    //                     valid_boses.Add(mini_boss_types[i]);
    //                     valid_dif.Add(boss_dif[i]);
    //                 }
    //             }
    //         }
    ...
    //             int choice = Random.Range(0, valid_boses.Count);
    //             manager.CurrentBossDif -= valid_dif[choice]; 
                
    //             //has_appeared_this_round[choice] = true;
    //             //boss_spawn_cooldown[choice] = boss_cooldown[choice];
    ...
    // This returns a list for valid entities in certain (fire - phoenix) rounds.

    //public List<GameObject> FireRoundList
    //{
    //    get
    //    {
    //        List<GameObject> result = UpdatedList;
    //
    //        for (int i = 0; i < enemy_types.Length; i++)
    //        {
    //            if (enemy_types.GetComponent<EnemyClass>().exclusive_round != fire))
    //            {
    //                result.Remove(enemy_types[i]);
    //            }
    //        }
    //
    //        return result;
